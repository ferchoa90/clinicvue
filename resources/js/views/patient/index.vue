<template>
  <div class="app-container">
    <el-table ref="vuetable" v-loading="loading" :data="patients" border fit highlight-current-row style="width: 100%">
...
export default {
  ...
  methods: {
    ...
    handleDelete(id, name, index) {
      this.$confirm('Está seguro de borrar al paciente (' + name + '), ¿Continuar?', 'Confirmación', {
        confirmButtonText: 'OK',
        cancelButtonText: 'Cancel',
        type: 'warning',
      }).then(() => {
        axios.get('/patient/delete/' + id)
          .then(response => {
            this.$message({
              type: 'success',
